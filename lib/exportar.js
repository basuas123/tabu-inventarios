...
// ═══════════════════════════════════════════════════════════════════════════════
export function exportarParaSoft({ sucursal, revisiones, semana, año }) {
  const wb = XLSX.utils.book_new()

  const rows = [
    ['AJUSTE PARA SOFT RESTAURANT — ' + (sucursal || '').toUpperCase(), '', ''],
    ['Semana: ' + semana + '   |   Año: ' + año + '   |   Generado: ' + new Date().toLocaleDateString('es-MX'), '', ''],
    ['Solo productos con estatus CORREGIDO', '', ''],
    ['', '', ''],
    ['PRODUCTO', 'CANTIDAD AJUSTADA', 'NOTAS'],
  ]
  ...
  rows.push(['', '', ''])
  rows.push(['Total productos a ajustar: ' + revisiones.length, '', ''])

  const ws = XLSX.utils.aoa_to_sheet(rows)
  ws['!cols'] = [{ wch: 44 }, { wch: 20 }, { wch: 30 }]
...
export function exportarCobro({ sucursal, items, total, semana, año }) {
  ...
  const rows = [
    ...
    ['', '', '', ''],
    ['PRODUCTO', 'DIFERENCIA', 'MONTO ($)', 'NOTAS'],
  ]

  items.forEach(r => {
    rows.push([
      r.producto || '',
      r.impacto != null ? parseFloat(parseFloat(r.impacto).toFixed(3)) : 0,
  ...
  const ws = XLSX.utils.aoa_to_sheet(rows)
  ws['!cols'] = [{ wch: 44 }, { wch: 14 }, { wch: 16 }, { wch: 30 }]

  const range = XLSX.utils.decode_range(ws['!ref'])
  for (let R = 5; R <= range.e.r; R++) {
    const cMonto = XLSX.utils.encode_cell({ r: R, c: 2 })
