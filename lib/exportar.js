import { ordenarComoSoft, compararSoft } from './grupos'
...
export function exportarAnalisisSoft({ sucursal, resultados, totalFalt, totalSobr, neto, semana, año, softData }) {
  ...
  // Productos en el mismo orden que en pantalla (grupos como Soft)
  ordenarComoSoft(resultados).forEach(r => {
    rows.push([
      r.nombre,
      r.grupo    || '',
...
export function exportarParaSoft({ sucursal, revisiones, analisisSuc, semana, año }) {
  ...
  const detalle = ordenarComoSoft(analisisSuc?.detalle || [])
  ...
  // Filas con banda de grupo, como en pantalla
  const filasGrupo = []  // índices (0-based en rows) de las bandas, para formato
  let grupoActual = null
  productosCompletos.forEach(p => {
    const g = p.grupo || 'SIN GRUPO'
    if (g !== grupoActual) {
      grupoActual = g
      filasGrupo.push(rows.length)
      rows.push([g, '', '', '', '', ''])
    }
    rows.push([
      p.nombre,
  ...
  // Pintar bandas de grupo (azul como en pantalla)
  filasGrupo.forEach(R => {
    const a = XLSX.utils.encode_cell({ r: R, c: 0 })
    if (ws[a]) ws[a].s = { fill: { fgColor: { rgb: '2E75B6' } }, font: { color: { rgb: 'FFFFFF' }, bold: true } }
  })
...
export function exportarCobro({ sucursal, items, total, semana, año }) {
  ...
  const itemsOrdenados = [...items].sort((a, b) => compararSoft(
    { grupo: a.grupo, nombre: a.producto },
    { grupo: b.grupo, nombre: b.producto }
  ))
...
// Reporte Excel del análisis cargado + estatus de revisiones por producto
// ═══════════════════════════════════════════════════════════════════════════════
export function exportarRevisionCobro({ sucursal, analisisSuc, revisiones, semana, año }) {
  ...
  ;(revisiones || []).forEach(r => {
    revMap[(r.producto || '').toUpperCase()] = r
  })

  const detalle = (analisisSuc?.detalle || []).filter(d => d.resultado !== 'OK')

  const rows = [
    ['REVISIÓN Y COBRO — TABU SUSHI', '', '', '', '', '', ''],
    ['Sucursal: ' + (sucursal || 'TODAS').toUpperCase(), '', '', '', '', '', ''],
    ['Semana: ' + semana + '   |   Año: ' + año + '   |   Generado: ' + new Date().toLocaleDateString('es-MX'), '', '', '', '', '', ''],
    ['', '', '', '', '', '', ''],
    ['PRODUCTO', 'GRUPO', 'DIFERENCIA', 'IMPACTO ($)', 'RESULTADO', 'ESTATUS', 'NOTAS'],
  ]

  if (detalle.length > 0) {
    ordenarComoSoft(detalle)
      .forEach(d => {
        const rev = revMap[(d.nombre || '').toUpperCase()] || {}
        rows.push([
          d.nombre || '',
          d.grupo || '',
          d.dif != null ? parseFloat(parseFloat(d.dif).toFixed(3)) : 0,
          d.imp != null ? parseFloat(parseFloat(d.imp).toFixed(2)) : 0,
          d.resultado || '',
    ...
    // Sin análisis cargado: exportar solo las revisiones registradas
    ;(revisiones || []).forEach(r => {
      rows.push([
        r.producto || '',
        '',
        '',
        r.impacto != null ? parseFloat(parseFloat(r.impacto).toFixed(2)) : 0,
        '',
        r.estatus || '',
        r.notas || '',
      ])
  ...
  const totalImp = rows.slice(5).reduce((a, r) => a + (typeof r[3] === 'number' ? r[3] : 0), 0)
  rows.push(['', '', '', '', '', '', ''])
  rows.push(['IMPACTO TOTAL', '', '', parseFloat(totalImp.toFixed(2)), '', '', ''])

  const ws = XLSX.utils.aoa_to_sheet(rows)
  ws['!cols'] = [{ wch: 44 }, { wch: 20 }, { wch: 12 }, { wch: 14 }, { wch: 12 }, { wch: 14 }, { wch: 30 }]
  ...
  XLSX.utils.book_append_sheet(wb, ws, 'Revisión y cobro')
  XLSX.writeFile(wb, 'RevisionCobro_' + (sucursal || 'Todas').replace(/ /g, '_') + '_Sem' + semana + '_' + año + '.xlsx')
}
